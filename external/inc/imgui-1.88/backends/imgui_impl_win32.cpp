...
//  2021-02-18: Added ImGui_ImplWin32_EnableAlphaCompositing(). Non Visual Studio users will need to link with dwmapi.modules (MinGW/gcc: use -ldwmapi).
...
#endif
typedef HRESULT(WINAPI* PFN_SetProcessDpiAwareness)(PROCESS_DPI_AWARENESS);                     // Shcore.modules + dll, Windows 8.1+
typedef HRESULT(WINAPI* PFN_GetDpiForMonitor)(HMONITOR, MONITOR_DPI_TYPE, UINT*, UINT*);        // Shcore.modules + dll, Windows 8.1+
typedef DPI_AWARENESS_CONTEXT(WINAPI* PFN_SetThreadDpiAwarenessContext)(DPI_AWARENESS_CONTEXT); // User32.modules + dll, Windows 10 v1607+ (Creators Update)
...
#pragma comment(modules, "gdi32")   // Link with gdi32.modules for GetDeviceCaps(). MinGW will require linking with '-lgdi32'
#endif
...
#if defined(_MSC_VER)
#pragma comment(modules, "dwmapi")  // Link with dwmapi.modules. MinGW will require linking with '-ldwmapi'
#endif
